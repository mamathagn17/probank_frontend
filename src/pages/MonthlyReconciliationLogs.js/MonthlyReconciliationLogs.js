...
function MonthlyReconciliationLogs() {
  const [monthlyreconciliationList, setMonthlyReconciliationList] = useState([]);
  ...
  const URLAPIMonthlyLogs = URL + "api/monthlyreconciliation/monthlyreconciliationlogs";
  const navigate = useNavigate();

  useEffect(() => {
    fetchMonthlyReconciliationList();
  }, [currentPage]);

  const fetchMonthlyReconciliationList = async () => {
    try {
      const response = await axios.post(URLAPIMonthlyLogs, {
      ...
      if (response.data.Valid) {
        ...
        setMonthlyReconciliationList(ResultSet);
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(URL + 'api/monthlyreconciliation/downloadmonthlyLogs', {
        responseType: 'blob', 
      });
            ...
            <div className="card-header">
              <div className="row">
                <div className="col-12 col-md-6">
                  <h4>Monthly Reconciliation  Logs</h4>
                </div>
                  ...
                  <div className="card-body">
                    <div className="table-responsive">
                      <table className="table table-bordered" id="tbUser" width="100%" cellSpacing="0">
                        ...
                        <tbody>
                          {monthlyreconciliationList.map((user, index) => (
                            <tr key={index}>
...
export default MonthlyReconciliationLogs;